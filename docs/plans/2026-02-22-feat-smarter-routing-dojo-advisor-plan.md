...
This plan hardens routing without changing core teaching content:
- Add a deterministic query-type classifier above current mode/pattern cascade
- Add compare mode for exactly 2 explicit patterns
- Add one-turn follow-up context resolution (dojo/advisor envelope bridge)
- Add envelope v2 with explicit contract and compatibility shim
- Keep scope narrow; defer speculative capabilities to v2 backlog
...
### Hardening Decisions (pre-spec lock)

This plan is constrained to one shippable v1 slice:
- Query-type classifier (single, compare, follow-up, structured follow-up, disambiguation)
- Compare mode (exactly 2 patterns)
- 1-message context resolution (dojo + advisor envelope bridge)
- Envelope v2 normalization (`patterns_selected` always a list)

Out of scope for this spec draft:
- Pairings registry and pairing-specific synthesis sections
- 3+ pattern compare or sequential multi-pattern delivery
- New routing infrastructure beyond prompt/rule updates

Evidence gate before implementation: include at least 3 concrete failing prompts per improvement area (compare, follow-up, cross-skill handoff) and the expected fixed route.
...
| `--context-pattern=<slug>` flag present (repeatable) | structured follow-up | Extract one or more explicit slugs directly, skip pronoun resolution |
| Pronoun/reference without pattern name | follow-up | `$ARGUMENTS` contains "that one", "those", "it", ordinal refs ("first/second/third") AND no explicit pattern slug/alias resolved |
...
- **`--context-pattern=<slug>` repeatable flag added** for agent parity. Multiple values support compare without adding a second flag shape.
...
**1-message lookback is sufficient:** the primary use case is immediate follow-up after the previous assistant response. Include minimal ordinal handling (`first/second/third`) when advisor ranked output is present; broader multi-message resolution is out of scope.
...
/dojo compare --context-pattern=task-dag --context-pattern=wave-computation
```
...
**Precedence contract (authoritative):**
...
| Pronoun follow-up resolves to advisor ranked list + ordinal intent | Resolve ordinal (`first|second|third`) if present, else default to first ranked pattern |
...
Standardize on `patterns_selected` (always a list) and add `envelope_version` to both envelope types.
...
| `query_type` | string | All improvements | Always (resolved/final route: single, compare, follow-up, disambiguation) |
...
**Compatibility window (one release cycle):** emit both fields in dojo envelope:
- `patterns_selected` (authoritative, list)
- `pattern_selected` (legacy shim = first item of `patterns_selected`)

Remove `pattern_selected` after one release cycle. This de-risks parser drift and stop-hook breakage.
...
**Canonical contract location requirement:** add a runtime-facing envelope contract section to both skill docs (`agentic-dojo/SKILL.md` and `pattern-advisor/SKILL.md`) so parser expectations are not only documented in plan prose.
...
6. Emit `patterns_selected` (list) plus one-cycle `pattern_selected` compatibility shim
7. Add compare to zero-state, argument-hint, and examples
8. Update dojo stop-hook schema contract before rollout
...
- `/dojo compare --context-pattern=task-dag --context-pattern=wave-computation` routes to compare
...
- [ ] `--context-pattern=<slug>` is the only structured flag shape (repeatable for compare)
- [ ] Condensed semantic signal guidance in advisor catches structural descriptions that keyword matching misses
- [ ] Zero-state, argument-hint, and examples updated to show compare and structured follow-up usage
...
- [ ] `query_type` is defined as resolved/final route semantics
- [ ] `patterns_selected` is always a list (even single-element)
- [ ] One-cycle compatibility shim for `pattern_selected` is documented and tested
...
- [ ] Edge cases tested: same pattern twice, 3+ patterns in compare, empty context, invalid `--context-pattern`, ordinal follow-up (`first/second/third`), similarity-language compare intent
- [ ] Envelope extraction regex documented for observability pipeline consumers
- [ ] Dojo stop-hook contract updated to accept envelope v2 + compatibility shim
- [ ] Success metric defined pre-build (target reduction in clarification/error responses over a fixed prompt sample)
...
| 41 | `/dojo compare --context-pattern=task-dag --context-pattern=wave-computation` | structured follow-up | compare | task-dag, wave-computation |
| 42 | `/advisor "CI pipeline: lint, test, build, deploy"` | N/A (advisor) | advisor | task-dag (high), wave-computation (medium) |
| 43 | `/dojo what do wave and dag have in common?` | compare | compare | wave-computation, task-dag |
| 44 | "explain the second one" (after advisor ranked output) | follow-up | sensei | ranked_patterns[1] |
| 45 | `/dojo explain --context-pattern=not-a-real-slug` | structured follow-up (error) | N/A | Unknown context pattern error |
...
- Parent plan: `docs/plans/agentic-dojo-skill/2026-02-21-feat-agentic-dojo-skill-plan.md`
- Dojo skill target: `.claude/skills/agentic-dojo/SKILL.md`
- Advisor skill target: `.claude/skills/pattern-advisor/SKILL.md`
