...
**Introduced in: Stage 2 (updated in Stage 3)**

**Purpose:** Technical reference for wave algorithm details, spec file format, idempotency rules, retry protocol, fast path rules, clarifying question heuristics, token estimation, and observability events. `SKILL.md` delegates to this document for the mechanics of multi-task decomposition and execution.
